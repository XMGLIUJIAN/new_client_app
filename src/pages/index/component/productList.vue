<template>
	<view class="container mt-[20rpx] mb-[175rpx] mx-[30rpx] pt-[4rpx] px-[4rpx]">
		...
		<view class="article_product">
            <Product v-for="(item,index) in selectedData" :key="index" :tt="1" :day="item.day_cash_rate" @tap="navigateTo(`/pages/product_info/product_info?type=${item.country}`)"/>
		</view>
	</view>
</template>
...
<script lang="ts" setup>
    ...
    const selectedData = computed(() => {
        return props.data.filter(item =>
            selectActive.value === '' || item.country === selectActive.value
        );
    });
