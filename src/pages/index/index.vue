<template>
	<view class="container">
		...
		<view class="index">
			...
			<InformasiAset v-if="indexProduct && indexProduct.is_investing==false" :informasiInfo="indexProduct"></InformasiAset>
			<template v-for="item in state.pages" :key="item.name">
				<template v-if="item.name == 'middle-banner'">
					<w-middle-banner :content="item.content" :styles="item.styles" />
				</template>
			</template>
			<ProductList :productData="productListData"></ProductList>
		</view>
...
<script setup lang="ts">
    ...
    import { getProductApi, getProductDetailApi } from '@/api/product'
    ...
    const productListData = ref<Array<any>>([])
    ...
    const indexProduct = computed(()=>{
        return productListData.value.find(item => item.product_id === "P2025060710000") || null
    })
    ...
    const getProductListData = async () => {
        ...
        productListData.value = data.lists
