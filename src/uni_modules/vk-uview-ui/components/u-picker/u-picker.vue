...
<template>
	<u-popup :maskCloseAble="maskCloseAble" mode="bottom" :popup="false" v-model="popupValue" length="auto"
		:safeAreaInsetBottom="safeAreaInsetBottom" @close="close" :z-index="uZIndex" :blur="blur">
		<view class="u-datetime-picker">
			<view class="u-picker-header" @touchmove.stop.prevent="stop">
				<view class="u-btn-picker u-btn-picker--tips" :style="{ color: cancelColor }" hover-class="u-opacity"
					:hover-stay-time="150" @click="getResult('cancel')">
					{{ cancelText }}
				</view>
				<view class="u-picker__title">{{ title }}</view>
				<view class="u-btn-picker u-btn-picker--primary" :style="{ color: moving ? cancelColor : confirmColor }"
					hover-class="u-opacity" :hover-stay-time="150" @touchmove.stop="stop"
	...
	export default {
		...
		methods: {
			...
			initTimeValue() {
				...
				fdate =
					`${dateObj.year}/${dateObj.month}/${dateObj.day} ${dateObj.hour}:${dateObj.minute}:${dateObj.second}`;
				// 时间字符串处理结束-----------------------------------------------------------
