<template>
	<u-popup v-model="showPay" mode="bottom" safe-area-inset-bottom :mask-close-able="false" border-radius="14"
		...
		<view class="h-[900rpx]">
			<page-status :status="popupStatus" :fixed="false">
				<template #error>
					<u-empty text="Informasi pesanan salah, informasi pesanan tak dapat dilacak" mode="order"></u-empty>
				</template>
				<template #default>
								...
								<view class="payway-lists">
									<u-radio-group v-model="payWay" class="w-full">
										<view class="p-[20rpx] flex items-center w-full payway-item"
											...
											<u-radio activeColor="var(--color-primary)" class="mr-[-20rpx]"
												:name="item.pay_way">
											</u-radio>
										</view>
									</u-radio-group>
	...
	<u-popup class="pay-popup" v-model="showCheckPay" round mode="center" borderRadius="10" :maskCloseAble="false">
		<view class="content bg-white w-[560rpx] p-[40rpx]">
			<view class="text-2xl font-medium text-center"> Pembayaran dikonfirmasi </view>
			<view class="pt-[30rpx] pb-[40rpx]">
				<view> Lewat wechat. Jika kau sudah membayar lewat email </view>
			</view>
			<view class="flex">
...
<script lang="ts" setup>
	...
	*/
	...
	const queryPayResult = async (confirm = true) => {
		...
		if (res.pay_status === 0) {
			if (confirm == true) {
				emitter.emit('toast', 'Pesanan anda belum dibayar, mohon dibayar ulang')
			}
			showPay.value = true
			handlePayResult(PayStatusEnum.FAIL)
		} else {
			if (confirm == false) {
				emitter.emit('toast', 'Perintahmu sudah dibayar, jangan dibayar ulang')
			}
